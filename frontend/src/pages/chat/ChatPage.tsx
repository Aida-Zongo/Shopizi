...
export default function ChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col">
      <div className="mb-4">
        <h1 className="text-headline-sm font-headline-sm text-text-main">Messages</h1>
        <p className="text-body-md text-text-muted mt-1">Conversations avec vos clients</p>
      </div>
      ...
      <div className="flex-1 flex bg-surface-container-lowest rounded-xl border border-outline-variant/20 overflow-hidden shadow-[0px_4px_20px_rgba(0,0,0,0.04)]">
        ...
        <div className="w-full md:w-80 border-r border-outline-variant/20 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            {rooms.length === 0 ? (
              <div className="p-8 text-center text-text-muted text-body-md">Aucune conversation</div>
            ) : (
              rooms.map(room => (
                <button
                  key={room.id}
